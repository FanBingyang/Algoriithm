 ...
 * ·标签：单调队列、辅助队列
 * ·整体思路：
 *      ·这道题和 MaxSlidingWindow 的思路很像，最难的地方在于取出最大值函数的时间复杂度如何降为 O(1)O(1)，如果每次入队的时候都维护一个最大值，那么时间复杂度很明显不满足条件
...
public class MaxQueue {
    ...
    public void push_back(int value) {
        // add(E e):添加一个元素到队尾，成功返回true，如果元素为空，则发生NPE(空指针异常)
        // offer(E e):添加一个元素到队尾，成功返回true，事败返回false。
        queue.offer(value);
